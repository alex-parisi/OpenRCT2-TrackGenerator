...
The add-on bundles four kinds of wheel:
  ...
  4. The front-end wheel (``openrct2_trackgenerator``): this repo, now
...
Run from the repo root (pip must be importable):
    ...
    uv run --with pip python scripts/collect_wheels.py --addon track
"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--addon", choices=ADDONS, default="track", help="which add-on's wheels/ to populate"
    )
    args = ap.parse_args()
